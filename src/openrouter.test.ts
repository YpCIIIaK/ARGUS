...
import { currentDateTimeContext, openRouterEffort } from "./openrouter.js";
...
test("maps max effort to OpenRouter xhigh", () => {
  assert.equal(openRouterEffort("max"), "xhigh");
  assert.equal(openRouterEffort("medium"), "medium");
});
